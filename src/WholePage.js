...
function WholePage() {
  ...
  const barData = {
    labels: kowordrankData.map(item => item['제목']),
    datasets: [
      {
        label: '기사 점수',
  ...
  const barOptions = {
    ...
    plugins: {
      title: {
        display: true,
        text: `KoWordRank 모델 결과 - ${category}`,
        color: 'white',
        font: { size: 18 },
      },
    ...
    scales: {
      ...
      y: {
        title: { display: true, text: '기사 제목', color: 'white' },
    ...
    onClick: (event, elements) => {
      if (elements && elements.length > 0) {
        ...
        const link = kowordrankData[index].링크;
        if (link) {
          window.open(link, '_blank');
        } else {
          alert("해당 기사로 이동할 수 없습니다.");
        }
      }
    }
  };
