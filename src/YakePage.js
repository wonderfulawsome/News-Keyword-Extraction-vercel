...
function YakePage() {
  ...
  useEffect(() => {
    fetch(`https://news-keyword-extraction.onrender.com/yake?category=${selectedCategory}`)
      .then(res => res.json())
      .then(data => {
        // data: { "키워드": {score, link}, ... } → 배열로 변환
        const yakeArray = Object.keys(data).map(key => ({
          keyword: key,
          score: data[key].score,
          link: data[key].link
        }));
        // YAKE는 낮은 점수가 더 중요하므로 오름차순 정렬
  ...
  const barOptions = {
    ...
    plugins: {
      title: {
        display: true,
        text: `YAKE 모델 키워드 점수 - ${selectedCategory}`,
        color: 'white',
        font: { size: 18 }
      },
      legend: {
  ...
  // 네비게이션 바: 각 카테고리 링크
  const categories = ["전체", "정치", "경제", "사회", "세계", "문화", "연예", "스포츠"];
    ...
    <div className="container">
      <div className="navbar">
        ...
        <div className="nav-links">
          {categories.map(cat => (
            <Link key={cat} to={`/yake/${cat}`} className="button" style={{ marginLeft: '10px' }}>
              {cat}
            </Link>
          ))}
        </div>
      ...
      <div className="header" style={{ textAlign: 'center' }}>
        <h1 className="title">YAKE 모델 키워드 결과 - {selectedCategory}</h1>
      </div>
      {loading ? (
